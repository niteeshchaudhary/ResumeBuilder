...
def send_expiring_email(user):
    ...
    try:
        ...
        html_message = render_to_string('emails/plan_expiring.html', context)
        plain_message = render_to_string('emails/plan_expiring.html', context)
...
def send_welcome_email(user):
    ...
    html_content = render_to_string('emails/welcome.html', {'username': user.email.split("@")[0]})
...
def send_password_reset_email(user, token):
    ...
    html_content = render_to_string('emails/password_reset.html', {'token': token})
...
def send_advertisement_email():
    ...
    for user in users:
        ...
        html_content = render_to_string("emails/advertisement.html", {'user':user.email})
...
def send_verification_email(user, request):
    ...
    verification_link = f"{os.getenv('FRONTEND_URL','https://resumeupgrader.in')}/verifyemail/{uid}/{token}/"
...
def send_within_org(data,request):
    
    subject = 'Someone tried to connect with you on Reserish'
    from_email = None
    to_email = "resumeupgrader.rn@gmail.com"
    email = EmailMultiAlternatives(subject, data, from_email, [to_email])
    email.send()
