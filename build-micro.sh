...
# Build micro Celery images for maximum size reduction

echo "🚀 Building micro Celery images..."
...
# Build micro Celery worker
echo "🔨 Building micro Celery worker..."
docker build \
  --no-cache \
  --build-arg BUILDKIT_INLINE_CACHE=1 \
  -f Dockerfile.celery.micro \
  -t reserish_main_celery:latest \
  .

# Build micro Celery beat
echo "🔨 Building micro Celery beat..."
docker build \
  --no-cache \
  --build-arg BUILDKIT_INLINE_CACHE=1 \
  -f Dockerfile.celery.micro \
